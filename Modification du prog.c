...
// Broche pour le capteur de fin de course
const int pinFinDeCourse = 51;
...
/* ----------------------------------------------- */
  ...
  pinMode(pinFinDeCourse, INPUT_PULLUP);
...
void loop() {
  ...
  // Si le deuxième bouton est enfoncé, activer le moteur à pleine vitesse dans le sens antihoraire
    ...
    analogWrite(pinEnableB, 85);  // Pleine vitesse (255 correspond à 100%)
...
////////////////////////////////////////////////////////////////
////// BOUT DU PROGRAMME PERMETTANT INCLUANT LE CAPTEUR ////////
////// DE FIN DE COURSE DU COUDE AFIN QUE LE BOUTON M3  ////////
////// SOIT IMPOSSIBLE A UTILISER TANT QUE LE CAPTEUR EST //////
////// ACTIVE //////////////////////////////////////////////////
////////////////////////////////////////////////////////////////

  // Lecture de l'état du capteur de fin de course
  int etatFinDeCourse = digitalRead(pinFinDeCourse);

  // Si le capteur de fin de course est enfoncé, désactiver le contrôle du moteur vers le bas du coude
  if (etatFinDeCourse == LOW) {
    digitalWrite(pinInput5, LOW);  // Sens antihoraire
    digitalWrite(pinInput6, HIGH);
    //buttonState6_M3_BAS = HIGH; // Désactiver le bouton pour descendre le coude
  } else {
    // Lire l'état du bouton pour descendre le coude
    buttonState6_M3_BAS = digitalRead(pinButton_M3_BAS);

    if (buttonState6_M3_BAS == LOW) {
      analogWrite(pinEnableB, 85);  // Pleine vitesse (255 correspond à 100%)
...
///////////////////////////////////////////////////////////////////////////
   ...
   //digitalWrite(pinFinDeCourse, LOW) ;
  }
  }
}
